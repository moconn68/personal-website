...
| personal-website | T-3 | 2026-09-22 | T-3 | `getSections()` helper + typed section skeletons: `src/config/sections.ts` (sorted ascending by `order`, fail-fast id≡slug, `sectionPath()` home→`/`, re-export of TEMPLATES); `home`/`resume`/`about` skeleton content files with `HUMAN COPY` placeholders (zero invented facts); build/`astro check` clean; review Approve; QA Pass (report: docs/qa/qa-report-personal-website-T-3.md). Completion Log row stores the T-ID (the committed row cannot reference its own hash — hash discoverable via `git log`) |
| personal-website | T-4 | 2026-09-22 | T-4 | Site URL config: `src/config/site.ts` per design §4.5 (SITE_URL env-override + trailing-slash strip, SITE_ORIGIN, `path()`/`absoluteUrl()` single trailing-slash policy), `astro.config.mjs` `site` key (`process.env.PUBLIC_SITE_URL ?? 'https://mattoconn.pages.dev'`), `.env.example` documenting `PUBLIC_SITE_URL` + `CF_PAGES_BRANCH` with `CF_PAGES_URL`-never-a-site-URL constraint; builds exit 0 unset/override/trailing-slash; review Approve; QA Pass (report: docs/qa/qa-report-personal-website-T-4.md, watch-item F1 for T-15/T-19). Completion Log row stores the T-ID (the committed row cannot reference its own hash — hash discoverable via `git log`) |
| personal-website | T-5 | 2026-09-22 | T-5 | Registry-driven nav: `src/components/Nav.astro` — `<nav aria-label="Main">` iterating `getSections()`, hrefs via `sectionPath()` (design §4.4 supersedes ticket's literal `/{slug}`), active state `Astro.url.pathname === sectionPath(entry)` with `aria-current="page"` + accent/underline (never color alone), scoped CSS with byte-exact token fallbacks, mobile stack / desktop `space-between` (UI §4.1), zero JS/hardcoded links; deviations noted inline (href policy, desktop alignment, non-active link color); `astro check` 0 errors, build green, negative `rg` gates pass; review Approve (1 minor alignment finding closed in same pass); QA Pass (report: docs/qa/qa-report-personal-website-T-5.md, watcher W-2 → T-7 trailingSlash). Completion Log row stores the T-ID (the committed row cannot reference its own hash — hash discoverable via `git log`) |
| personal-website | T-6 | 2026-09-22 | T-6 | Shared chrome: `src/layouts/BaseLayout.astro` (`<html lang="en">`, charset+viewport, named head slot for T-12 Seo, skip-link → `#main`, `<header><Nav /></header>` from T-5, `<main id="main"><slot /></main>`, `<footer>` copyright from the registry home entry — never hardcoded) + `src/assets/styles/global.css` (byte-exact §6.2–§6.4 tokens, minimal reset, global focus ring §6.5, reduced-motion guard; `@font-face` reserved for T-17); zero `<script>`/props; deviations noted inline (no `path` prop — vestigial per §5.1; `<nav>` landmark via Nav.astro; `text-size-adjust` + heading baseline adds); `astro check` 0 errors/0 warnings, build green; review Approve; QA Pass (report: docs/qa/qa-report-personal-website-T-6.md, watchers W-1 favicon → T-7 per design §12.5, W-2 title transiency → T-12, W-6 home h1 `--lh-hero` → T-8). Completion Log row stores the T-ID (the committed row cannot reference its own hash — hash discoverable via `git log`) |